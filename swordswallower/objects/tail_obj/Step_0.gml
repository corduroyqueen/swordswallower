...
if abs(player_obj.camx+640-x)>710 ||
abs(player_obj.camy+360-y)>430 {
	outside = true
	player_obj.gem_active = false
	player_obj.gem_thrown = false
} else {
...
if player_obj.tail_pulling {
	...
	if (distance_to_object(player_obj)<10 && pull_timer>pull_wall_t) {
		player_obj.tail_pulling=false
	...
	if floor(pull_timer)==floor(pull_wall_t) && !inside_flier {
		held_image_angle = image_angle	audio_manager(gsound.s_sword_whoosh,0,true,0)	
...
if player_obj.tail_held {
	...
	if wiggle_timer>0 {
		wiggle_timer--	
		image_angle = held_image_angle + random_range(-5,5)
	}
...
if player_obj.tail_planted {
	...
	//player_obj.tail_dest_x = x
	//player_obj.tail_dest_y = y
	if abs(player_obj.camx+640-x)>1000 ||
	abs(player_obj.camy+360-y)>600 {
	...
	//sdm(point_distance(player_obj.x,player_obj.y,player_obj.tail_dest_x,player_obj.tail_dest_y))
	//sdm(point_distance(player_obj.x,player_obj.y,player_obj.tail_dest_x,player_obj.tail_dest_y))
	if point_distance(player_obj.x,player_obj.y,player_obj.tail_dest_x,player_obj.tail_dest_y)<30
	...
	} else if point_distance(player_obj.x,player_obj.y,player_obj.tail_dest_x,player_obj.tail_dest_y)<110 
	&& !inside_flier && current_obj!=dash_guy_obj  && current_wall.object_index!=door_obj
	&& current_wall.object_index!=circle_friend_obj && !player_obj.zoom_timer_bool && pickup_timer<1
	{ 
		player_obj.tail_planted = false
		player_obj.tail_pulling = true
		/*
...
if player_obj.tail_throwing && !in_camera_range_bigger(x,y) {
	...
	if player_obj.tail_throwing {
		...
		if place_meeting(x,y,impale_circle_obj) || place_meeting(xpreva,ypreva,impale_circle_obj) {
			with player_obj {
				if place_meeting(x,y,instance_place(other.xpreva,other.ypreva,impale_circle_obj)) && point_distance(x,y,other.x,other.y)<200 {
		...
		if place_meeting(x,y,stinky_pile_obj) || place_meeting(xpreva,ypreva,stinky_pile_obj) {
			with player_obj {
				if place_meeting(x,y,instance_place(other.xpreva,other.ypreva,stinky_pile_obj)) && point_distance(x,y,other.x,other.y)<200 {
					other.temptemptemp = false
		...
		if place_meeting(x,y,circle_friend_obj) || place_meeting(xpreva,ypreva,circle_friend_obj) {
			with player_obj {
				if place_meeting(x,y,instance_place(other.xpreva,other.ypreva,circle_friend_obj)) && point_distance(x,y,other.x,other.y)<200 {
					other.temptemptemp = false
	...
	if player_obj.tail_throwing || player_obj.tail_pulling { 
		...
		if place_meeting(x,y,hummingbird_obj) {
			with player_obj {
				if place_meeting(x,y,hummingbird_obj) && point_distance(x,y,other.x,other.y)<100 {
					other.temptemptemp = false

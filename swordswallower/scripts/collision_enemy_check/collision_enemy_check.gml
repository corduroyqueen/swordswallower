...
if (player_obj.tail_throwing || player_obj.tail_pulling) && !local_obj.met && !player_obj.death {
	...
	if local_obj.object_index == boss_dada_obj {
		if local_obj._state == local_obj._state_guarddown {
			local_obj.can_sword_be_planted = true
		} else {
			sword_reject_script()
			x = oldx
			y = oldy
	...
	if local_obj.object_index == cryptkeepershielded_obj {
	...
	if local_obj.object_index == boss_knifegirl_obj {
		if local_obj._state == local_obj._state_guarddown {
			local_obj.can_sword_be_planted = true
		} else {
			sword_reject_script()
	...
	if local_obj.object_index==flyswarm_obj {
		...
		sword_plant_in_enemy(local_obj)

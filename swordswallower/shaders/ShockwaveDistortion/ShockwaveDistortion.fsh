...
void main() {
	...
	*/
	...
	//fromuti:change 0.5 and 1.5 to alter the amount of abberation
	// v3b: Chromatic Aberation orange/blue:------------------------------
	//out_col			=	(texture2D(gm_BaseTexture, v_vTexcoord + offset) +                   //vec4(vec3(1.0, 1.0, 1.0), 1.0)
	//					texture2D(gm_BaseTexture, v_vTexcoord + offset * (0.5)) * vec4(vec3(1.0, 0.5, 0.0), 1.0) +
	//					texture2D(gm_BaseTexture, v_vTexcoord + offset * (1.5)) * vec4(vec3(0.0, 0.5, 1.0), 1.0)) *
